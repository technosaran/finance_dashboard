...
import { inferDirectionFromSemanticType, normalizeTransactionType } from '@/lib/utils/transactions';
...
export const FinanceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  function serializeTransactionMetadata(metadata?: Transaction['metadata']): Json {
  ...
  const addTransaction = useCallback(
    async (transactionData: Omit<Transaction, 'id'>) => {
      const transactionType = normalizeTransactionType(
        ...
        String(transactionData.category)
      );
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          type: inferDirectionFromSemanticType(transactionType),
          transaction_type: transactionType,
          amount: transactionData.amount,
          account_id: transactionData.accountId || null,
          metadata: serializeTransactionMetadata(transactionData.metadata),
        })
        .select()
        .single();
        ...
        .update({
          ...
          type:
            transactionType && transaction.type
              ? inferDirectionFromSemanticType(transactionType)
              : transaction.type,
          transaction_type: transactionType,
          ...
          metadata:
            transaction.metadata === undefined
              ? undefined
              : serializeTransactionMetadata(transaction.metadata),
        })
        .eq('id', id);
        ...
        const { error } = await supabase.from('app_settings').upsert(
          {
            ...
            compact_numbers: updatedSettings.compactNumbers ?? false,
  ...
  const settingsValue = useMemo(
    () => ({ settings, updateSettings, loading, error, lastUpdatedAt }),
    [settings, updateSettings, loading, error, lastUpdatedAt]
  );
